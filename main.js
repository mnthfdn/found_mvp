...
var basketballPosition = function(sport, game, team, position, score, rebounds, assists) {
  this.sport = sport || '';
  this.game = game || '';
  this.team = team || '';
...
basketballPosition.prototype.calculatePoints = function() {
  var rules = {
...
var handballPosition = function(sport, game, team, position, made, received) {
  this.sport = sport || '';
  this.game = game || '';
  this.team = team || '';
  this.position = position || '';
...
handballPosition.prototype.calculatePoints = function() {
  var rules = {
...
var parseBasketball = function(game, rawGame) {
  ...
  while ((m = rPlayer.exec(rawGame)) !== null) {
    ...
    var basketballPosition = new basketballPosition('basketball', game, m[4], m[5]
      , parseInt(m[6]), parseInt(m[7]), parseInt(m[8]));
    player.position = basketballPosition;
    players.push(player);
  };
  return players;
};
...
var parseHandball = function(game, rawGame) {
  ...
  while ((m = rPlayer.exec(rawGame)) !== null) {
    var player = new Player(m[1], m[2], m[3]);
    var handballPosition = new handballPosition('handball', game, m[4], m[5]
      , parseInt(m[6]), parseInt(m[7]));
    player.position = handballPosition;
    players.push(player);
  };
  return players;
};
